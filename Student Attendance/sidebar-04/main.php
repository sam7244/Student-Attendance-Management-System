...
<?php
  ...
  if (isset($_POST['login']))
  {
         ...
         else
         {
            ...
            $subject=$row['name'];
                 $m= "select id from attendance_record where course='$course' and batch='$batch' and Faculty_Name='$fname' and semestar='$sem' 
                 and subject='$subject' and Date='$date' and class='$class' " ;
                 $r=mysqli_query($conn,$m);
                 if(mysqli_num_rows($r)>=1)
                 {
                     echo"<script>alert('Sorry Attendance is Already Taken !!!')</script>";
                 }
                 else
                 {
...
<form action ="" method="POST">
            <?php $mail=$_SESSION['email'];
                ...
                <div class="form-group">
                    ...
                    <input type="text"   name="course" list="course" value="<?php echo $courses;   ?>" placeholder="  Select Your course" class="col-lg-12 col-sm-12 col-12 p-2" style="outline:none;color:#000;border:none;text-indent:5px;background:#f1f1f1;cursor: not-allowed;" readonly >
                ...
                <div class="form-group">
                    ...
                    <input type="text"  name="fname"  value="<?php echo $name;   ?>" placeholder="  Select Your Name" class="col-lg-12 col-sm-12 col-12 p-2" style="outline:none;border:none;text-indent:5px;color:#000;background:#f1f1f1;cursor: not-allowed;" readonly > 
    ...
    <?php 
     ...
     ?>"   min="<?php
     echo date('Y-m-d');
 ?>"class="form-control border" id="exampleInputdate" required value="<?php 
 echo date('Y-m-d');
?>"   min="<?php
echo date('Y-m-d');
?>" required="required">
